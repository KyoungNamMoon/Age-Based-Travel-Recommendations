...
interface UniqueDestinationsProps {
  destinations: ApiDestination[];
}

export function UniqueDestinations({ destinations }: UniqueDestinationsProps) { 
      ...
      <div className="relative">
        ...
        >
          {destinations.map((destination, index) => (
            <UniqueDestinationCard key={index} destination={destination} />
          ))}
        </div>
...
function UniqueDestinationCard({ destination }: { destination: ApiDestination }) {
  const navigate = useNavigate();
  const [imageUrl, setImageUrl] = useState<string>(
    "https://images.unsplash.com/photo-1542718610-a1d656d1884c?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
  );
  const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:5000';

  useEffect(() => {
    if (!destination.name) return;
    fetch(`${API_URL}/api/gallery?keyword=${encodeURIComponent(destination.name + " " + destination.country)}&page=1`)
      .then(res => res.json())
      .then(photos => {
        if (Array.isArray(photos) && photos.length > 0) setImageUrl(photos[0]);
      })
      .catch(err => console.warn("Image fetch error:", err));
  }, [destination.name, destination.country, API_URL]);
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {destination.tags.map((tag, tagIndex) => (
            <span 
              key={tagIndex}
              className="px-3 py-1 bg-green-50 text-green-600 rounded-full text-xs"
            >
              {tag}
            </span>
          ))}
